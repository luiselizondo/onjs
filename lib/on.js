...
On.prototype._onEventReceived = function (topic, data) {
  this.__log(topic.eventName, 'Creating redis queue with name ' + topic.eventName)
  
  var now = Date.now()
  var jobId = data.tid || now
  var taskName = topic.eventName + '-' + now;

  this.redisQueue = new Queue(topic.eventName, {
    redis: this.config.redis
  })

  if (topic.dispatchAs) {
    this.__log(topic.eventName, 'Event is redispatchable')
    this._redispatchToMQ(topic, taskName)
  }
  else {
    this.__log(topic.eventName, 'Sending the event to redis as task ' + taskName)
    this._sendToRedisQueueToProcess(topic, taskName)
  }
...
On.prototype._sendToRedisQueueToProcess = function (queue, taskName) {
  ...
  this.redisQueue.process(taskName, (job, jobDone) => {
    ...
    this._log(taskName, 'Calling queue callback')

    return queue.callback(job.data, (err, result) => {
