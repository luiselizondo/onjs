  ...
  this.registerQueueEvents = function inititQueue() {
    this.queueInstances.forEach((instance) => {
      ...
      if (instance.isRequest) {
        eventsInstance.on(instance.getEventName(), (data) => {
          var callback = instance.getQueueCallback()
          return mqInstance.answerRequest(data.channel, data.message, callback)
        })
      }
      else {
        eventsInstance.on(instance.getEventName(), (data) => {
...
On.prototype.requestReceived = function (eventName) {
  this.instance = new Wrapper()
  this.instance.setEventName(eventName)
  this.instance.setIsRequest(true)
...
On.prototype.respond = function (callback) {
  this.instance.setCallback(callback)
  this.instance.willRespond = true
  return this
}
...
On.prototype.listen = function () {
  this.registerEvents()
  this.registerQueueEvents() // TODO: INtegrate both
  ...
  .then(() => {
	...
	.then(() => {
    this.mq.consumeFromQueue(this.eventsInstance.eventNames())
  })
  .then(() => {
    this.mq.consumeRequests(this.eventsInstance.eventNames())
  })
  .catch((err) => {
    console.log(err)
  })
