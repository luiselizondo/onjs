...
class On extends EventEmitter {
  ...
  async _initQueues (queues) {
    try {
      ...
      this.queueInstance.on('queueEventReceived', (data) => {
  ...
  _onTaskReceived (task, data) {
    this._eventLog('info', task.eventName, 'Executing task callback for task', task)
  ...
  _eventLog (type, eventName, message, data) {
    if (this.debugEnabled) {
      return this.logger[type](message)
    }
